...
import { IBlogs } from 'src/interfaces/blogs';
...
export const generateMapSite = async (
  ...
  const responseCampaings = await api.get<IBlogs>('Artigos/list/GetAll');

  const responseBlogs = await api.get<IBlogs>('blogs/list/GetAll');

  const fixedPages = ['/', '/sobre', '/industria','/produtos', '/blog'];

  const allObjectsOfCampaings = responseCampaings.data.posts.map((product) => ({
    url: '/campanhas/' + product.url,
    lastmod: dateFormated,
    priority: 0.9,
    changefreq: 'always',
  }));

  const allObjectsOfBlogs = responseBlogs.data.posts.map((product) => ({
